...
# Load YOLOv8 model
model = YOLO('park.pt')  # Adjust the model path as needed
...
def detect_car(frame):
    ...
    results = model(small_frame, conf=0.6, iou=0.3)  # Adjust conf and iou for better accuracy
    ...
    for result in results:
        for bbox in result.boxes:
            ...
            cv2.putText(frame, object_label, (x1, y1 + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2, cv2.LINE_AA)
    ...
    cv2.putText(frame, f"free: {available_spaces}", (7, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    # Write text to display the current date and time
    current_date = datetime.now().strftime("%Y-%m-%d")
    current_time = datetime.now().strftime("%H:%M:%S")

    # วางวันที่และเวลาในเฟรม
    cv2.putText(frame, current_date, (4, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 2)
    cv2.putText(frame, current_time, (4, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 2)
